import os, re, time, random, secrets, csv, io
...
if TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN:
    ...
    client = None
    print("WARNING: Twilio credentials not found. OTP functionality will be disabled.")
...
def send_otp(phone_number, otp):
    ...
    try:
        message = client.messages.create(
            body=f"Your verification code is: {otp}. Valid for 1 minute.",
...
def cleanup_events(silent=False):
    try:
        ...
        print(f"Cleanup events error: {e}")
        if not silent:
            flash('Error during cleanup. Please try again.', 'error')
            return redirect(url_for('host_dashboard'))
...
def cancel_registration(event_id):
    ...
    try:
        ...
        flash('Error cancelling registration. Please try again.', 'error')
        return redirect(url_for('student_dashboard'))
...
def host_dashboard():
    ...
    try:
        ...
        print(f"Host dashboard error: {e}")
        flash('Error loading dashboard. Please try again.', 'error')
        return redirect(url_for('login'))
...
def send_otp_route():
    try:
        ...
        # In debug mode or when Twilio is not configured, just log the OTP
        if app.debug or not client:
            print(f"DEBUG/NO_TWILIO - OTP for {phone_number} ({usertype}): {otp}")
            return jsonify({'success': True, 'message': f'OTP generated: {otp} (Debug mode or Twilio not configured)'})
...
def get_event_registrations(event_id):
    ...
    try:
        ...
        for reg in registrations:
            reg_dict = dict(reg)
            reg_dict['registration_date'] = reg['registration_date'].strftime('%Y-%m-%d %H:%M')
        ...
        return jsonify({'success': False, 'message': 'Error fetching registrations'})
...
def download_registrations(event_id):
    ...
    try:
        ...
        print(f"Download registrations error: {e}")
        flash('Error downloading registrations. Please try again.', 'error')
        return redirect(url_for('host_dashboard'))
...
def cancel_event(event_id):
    ...
    try:
        ...
        return jsonify({'success': False, 'message': 'Error cancelling event'})
...
def event_report(event_id):
    ...
    try:
        ...
        print(f"Event report error: {e}")
        flash('Error loading event report. Please try again.', 'error')
        return redirect(url_for('host_dashboard'))
...
def delete_past_event(event_id):
    ...
    try:
        ...
        print(f"Delete past event error: {e}")
        return jsonify({'success': False, 'message': 'Error deleting event record'})
...
def check_expired_events():
    ...
    try:
        ...
        print(f"Check expired events error: {e}")
        return jsonify({'success': False, 'message': 'Error checking expired events'})
...
def register_for_event(event_id):
    ...
    try:
        ...
        print(f"Register for event error: {e}")
